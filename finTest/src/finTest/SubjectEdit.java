...
public class SubjectEdit extends JFrame {
	...
	private SubjectEdit subjectFrame;
	...
	public SubjectEdit(SubjectTab tableFrame, Database db, StaticTab st) {
		...
		beforeName = nameTextField.getText();
		//profComboBox.setSelectedItem(data[2]);
	...
	private void setPanel() {
		...
		add(panel); // 만들어진 판낼을 프레임이 붙이는 부분
	...
	class ButtonListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getSource() == saveButton) {
				tableFrame.modifyRow(textField.getText(), nameTextField.getText(), findProNum());
				tableFrame.modifyProject(dayComboBox.getSelectedItem().toString(), changeLTime(), textField.getText());
				st.removeSet(beforeName);
